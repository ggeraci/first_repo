...
class particle
{
  ...
  void Send_Recv_Y();
  void Send_Recv_Z();
  void Send_Recv_X(particle&); //overload for InOut simulation
 ...
 public:
  ...
  //send and recv required buffer   public to handle sharing amongst box and duct
  double *Sbuf0;
  int Snum0;
  double *Sbuf1;
  int Snum1;
...
void part2gas_concentration(tensor0&); // compute number of particles per cell using linear projection
  ...
  void Send_Recv(particle&); //InOut simulation overload
  ///////////STATISTICS///////////
