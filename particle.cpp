...
particle::particle(params *p,proc *pc,gridsize *s)
{
  ...
  Buffer_max_size=N_max*N_max*N_max*particle_density*30; //this needs to be changed.... only for the case we change number of process and wanna load from restart files we need big buffers
...
double particle::Balance_Index()
{
  ...
  MPI_Bcast(&GlobalMax,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
  double GlobalMin;
  MPI_Reduce(&LocalNp,&GlobalMin,1,MPI_DOUBLE,MPI_MIN,0,MPI_COMM_WORLD);
...
void particle::Send_Recv_X(particle& pbox)
{
  MPI_Request request_send[2];
  MPI_Request request_recv[2];
  MPI_Status stat[2];
  //First get message length
  if (!pc_->IsOut()) MPI_Irecv(&Rnum0,1,MPI_INT,pc_->RIGHT(),21,MPI_COMM_WORLD,&request_recv[0]); else Rnum0=0;
  if (!pc_->IsIn()) MPI_Irecv(&Rnum1,1,MPI_INT,pc_->LEFT(),20,MPI_COMM_WORLD,&request_recv[1]); else Rnum1=pbox.Rnum1;
  SortValidX();
  if (!pc_->IsOut()) MPI_Isend(&Snum0,1,MPI_INT,pc_->RIGHT(),20,MPI_COMM_WORLD,&request_send[0]);
  if (!pc_->IsIn()) MPI_Isend(&Snum1,1,MPI_INT,pc_->LEFT(),21,MPI_COMM_WORLD,&request_send[1]);
  ...
  // Now send actual data
  if (!pc_->IsOut())  MPI_Irecv(Rbuf0,Rnum0,MPI_DOUBLE,pc_->RIGHT(),31,MPI_COMM_WORLD,&request_recv[0]);
  if (!pc_->IsIn()) MPI_Irecv(Rbuf1,Rnum1,MPI_DOUBLE,pc_->LEFT(),30,MPI_COMM_WORLD,&request_recv[1]);
  else //read incoming particles from box
      for (int i(0);i<Rnum1;i++)
	Rbuf1[i]=pbox.Rbuf1[i];
  if (!pc_->IsOut()) MPI_Isend(Sbuf0,Snum0,MPI_DOUBLE,pc_->RIGHT(),30,MPI_COMM_WORLD,&request_send[0]);
  if (!pc_->IsIn()) MPI_Isend(Sbuf1,Snum1,MPI_DOUBLE,pc_->LEFT(),31,MPI_COMM_WORLD,&request_send[1]);
...
void particle::Send_Recv(particle& pbox)
{
  Send_Recv_Z(); //first z
  Send_Recv_Y(); //then y
  Send_Recv_X(pbox); //then x
}
...
void particle::Store(char *name,int num_ts,double *p)
{
  ...
  std::string filename=filename_out_Data.str();
  MPI_File Myfile;
  MPI_File_open(MPI_COMM_WORLD,(char*)(filename.c_str()),MPI_MODE_CREATE|MPI_MODE_WRONLY|MPI_MODE_EXCL,MPI_INFO_NULL,&Myfile);
